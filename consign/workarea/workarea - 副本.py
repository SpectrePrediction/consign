...
def same_name_singleton(cls):
    ...
    def inner(name: str="DEFAULT_WORK_AREA"):
        """
        牺牲泛用性为代价适配修改了几处地方
            _instance_dict被移到了外面，这样导致所有same_name_singleton共用一个_instance_dict，但方便外部使用
            inner 参数添加了 name，去除了*args, **kwargs，这样导致使用same_name_singleton修饰的类都需要传入name这个参数，
                原来是无要求的，但其实是不严谨的
        """
